...
#define MB_UART_TXD     GPIO_NUM_16
#define MB_UART_RXD     GPIO_NUM_17
#define MB_UART_RTS     GPIO_NUM_23
...
static void* master_get_param_data(const mb_parameter_descriptor_t* param_descriptor)
{
    ...
    if (param_descriptor->param_offset != 0) {
       switch(param_descriptor->mb_param_type)
       {
        ...
        ESP_LOGW(TAG, "Wrong parameter offset for CID #%u", (unsigned)param_descriptor->cid);
...
void master_operation_func(void *arg)
{
    ...
    ESP_LOGE(TAG, "Start modbus test...");
            ...
            if ((err != ESP_ERR_NOT_FOUND) && (param_descriptor != NULL)) {
                ...
                if ((param_descriptor->param_type == PARAM_TYPE_ASCII) &&
                   ...
                   // Check for long array of registers of type PARAM_TYPE_ASCII
                    ...
                    if (err == ESP_OK) {
                        ESP_LOGE(TAG, "Characteristic #%u %s (%s) value = (0x%" PRIx32 ") read successful.",
                        ...
                        if (*(uint32_t*)temp_data_ptr != 0xAAAAAAAA) {
                            ...
                            *(uint32_t*)temp_data_ptr = 0xAAAAAAAA;
                            err = mbc_master_set_parameter(cid, (char*)param_descriptor->param_key,
                                                              (uint8_t*)temp_data_ptr, &type);
                            if (err == ESP_OK) {
                                ESP_LOGE(TAG, "Characteristic #%u %s (%s) value = (0x%" PRIx32 "), write successful.",
                                                param_descriptor->cid,
                                                param_descriptor->param_key,
                                                param_descriptor->param_units,
                                                *(uint32_t*)temp_data_ptr);
                            } else {
                                ESP_LOGW(TAG, "Characteristic #%u (%s) write fail, err = 0x%x (%s).",
                                                param_descriptor->cid,
                                                param_descriptor->param_key,
                                                (int)err,
                        ...
                        ESP_LOGW(TAG, "Characteristic #%u (%s) read fail, err = 0x%x (%s).",
                                        param_descriptor->cid,
                                        param_descriptor->param_key,
                                        (int)err,
                                        (char*)esp_err_to_name(err));
                    ...
                    if (err == ESP_OK) {
                        if ((param_descriptor->mb_param_type == MB_PARAM_HOLDING) ||
                            ...
                            ESP_LOGE(TAG, "Characteristic #%u %s (%s) value = %f (0x%" PRIx32 ") read successful.",
                                            param_descriptor->cid,
                                            param_descriptor->param_key,
                                            param_descriptor->param_units,
                                            value,
                            ...
                            if ((state & param_descriptor->param_opts.opt2) == param_descriptor->param_opts.opt2) {
                                ESP_LOGE(TAG, "Characteristic #%u %s (%s) value = %s (0x%" PRIx8 ") read successful.",
                                                param_descriptor->cid,
                                                param_descriptor->param_key,
                                                param_descriptor->param_units,
                                                (const char*)rw_str,
                                ...
                                ESP_LOGW(TAG, "Characteristic #%u %s (%s) value = %s (0x%" PRIx8 "), unexpected value.",
                                                param_descriptor->cid,
                                                param_descriptor->param_key,
                                                param_descriptor->param_units,
                                                (const char*)rw_str,
    ...
    if (alarm_state) {
        ESP_LOGE(TAG, "Alarm triggered by cid #%u.", param_descriptor->cid);
    } else {
        ESP_LOGW(TAG, "Alarm is not triggered after %u retries.", MASTER_MAX_RETRY);
    }
    //ESP_LOGE(TAG, "Destroy master...");
...
esp_err_t master_init(void)
{
    ...
    ESP_LOGE(TAG, "Modbus master stack initialized...");
    return err;
}
